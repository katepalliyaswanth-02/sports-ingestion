...
def create_job():
    ...
    try:
        ...
        mediaconvert = boto3.client(
            ...
            region_name=us-east-1,            # AWS region from configuration
            endpoint_url=https://sportapi7.p.rapidapi.com/api/v1/player/8171  # MediaConvert endpoint URL from configuration
        )

        # Define the S3 URL for the input video file to be processed
        input_s3_url = f"s3://{sports-highlights}/videos/first_video.mp4"

        # Define the S3 URL where the processed videos will be saved
        output_s3_url = f"s3://{sports-highlights}/processed_videos/"
        ...
        response = mediaconvert.create_job(
            Role=arn:aws:iam::307979338089:role/MediaConvertRole,                 # IAM role ARN that MediaConvert assumes
        ...
        # Then, when printing the response:
        try:
